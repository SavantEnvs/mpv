...
#       §6.2 item 16's `git clean -ffdX` (simulating rlenv's real patch-grader clean) only removes
#       GITIGNORED paths: the .wrap files survive (untracked but not ignored — `meson wrap install`
#       leaves them where a plain `git status` would flag them), but subprojects/packagecache/ and
#       every extracted subproject source dir ARE gitignored, so those vanish. WRAP_CACHE holds the
#       packagecache/ tarballs mayhem/Dockerfile snapshotted right after the first (networked)
#       build.sh run; restoring just that lets `meson subprojects download` re-extract everything
#       from local tarballs with no network, instead of re-fetching from WrapDB. ────────────────────
mkdir -p subprojects
...
if [ ! -d subprojects/packagecache ] && [ -d "$WRAP_CACHE/packagecache" ]; then
  mkdir -p subprojects/packagecache
  cp "$WRAP_CACHE"/packagecache/* subprojects/packagecache/
fi
